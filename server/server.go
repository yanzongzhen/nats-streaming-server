...
import (
	...
	"github.com/yanzongzhen/nats-streaming-server/spb"
...
type StanServer struct {
	...
	cliRequestSub *nats.Subscription
...
func (ss *subStore) updateState(sub *subState) {
	...
	if sub.isQueueSubscriber() {
		...
		if sub.ClientID == "" {
			...
			// we found in https://github.com/yanzongzhen/nats-streaming-server/issues/322
...
func RunServerWithOpts(stanOpts *Options, natsOpts *server.Options) (newServer *StanServer, returnedError error) {
	...
	topicGroup.Limit = sOpts.GroupLimit
...
func (s *StanServer) recoverOneSub(c *channel, recSub *spb.SubState, pendingAcksAsMap map[uint64]struct{},
	...
	if added || sub.IsDurable {
		// Repair for issue https://github.com/yanzongzhen/nats-streaming-server/issues/215
...
func (s *StanServer) initInternalSubs(createPub bool) error {
	...
	// Receive Client Info from clients
	s.cliRequestSub, err = s.createSub(DefaultRequestTopic, s.processNewJoinClient, "client Host upload")
...
func (s *StanServer) unsubscribeInternalSubs() {
	...
	if s.cliRequestSub != nil {
		s.cliRequestSub.Unsubscribe()
		s.cliRequestSub = nil
	}
}
...
func (s *StanServer) processNewJoinClient(m *nats.Msg) {
	...
	client := SyncClient{}
	if host, ok := resp["host"]; ok {
		client.Host = host.(string)
	}
	if port, ok := resp["port"]; ok {
		client.Port = int(port.(float64))
	}
	if topic, ok := resp["topic"]; ok {
		client.Topic = topic.(string)
	}
	if cid, ok := resp["client_id"]; ok {
		client.ClientID = cid.(string)
	}
	s.TopicGroupSnapshot.Add(&client, true)
	s.log.Noticef("current client : %v", client)
	s.log.Noticef("client list : %v", s.TopicGroupSnapshot.ToJson())
	cList := s.TopicGroupSnapshot.GetClientGroup(&client)
	if cList != nil {
		d, _ := json.Marshal(cList)
		s.log.Noticef("common group clients : %v", string(d))
		_ = s.ncs.Publish(m.Reply, d)
		key := fmt.Sprintf("T:%vG:%v", client.Topic, client.Group)
		_ = s.ncs.Publish(key, d)
		s.log.Noticef("notice %s group: data: %s", key, d)
	}
}
...
func (s *StanServer) processUnsubscribeRequest(m *nats.Msg) {
	...
	if client != nil {
		...
		cList := s.TopicGroupSnapshot.GetGroupClientsByGroup(group, req.Subject)
		s.log.Debugf("Sub : %s Group : %s", req.Subject, group)
		key := fmt.Sprintf("T:%vG:%v", req.Subject, group)
		if cList == nil {
			cList = make([]*SyncClient, 0)
		}
		d, _ := json.Marshal(cList)
		s.log.Debugf("client is list : %s", d)
		_ = s.ncs.Publish(key, d)
		_ = s.ncs.Flush()
	}
	if err != nil {
